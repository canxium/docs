---
title: JSON-RPC API
description: A stateless, light-weight remote procedure call (RPC) protocol for Canxium clients.
lang: en
---

In order for a software application to interact with the Canxium blockchain - either by reading blockchain data or sending transactions to the network - it must connect to an Canxium node.

For this purpose, every Canxium client implements a [JSON-RPC specification](https://github.com/ethereum/execution-apis), so there is a uniform set of methods that applications can rely on regardless of the specific node or client implementation.
...
When requests are made that act on the state of Canxium, the last default block parameter determines the height of the block.
...
Examples of using the JSON_RPC API by making [curl](https://curl.se) requests to an Canxium node are provided below. Each example
...
A handful of core JSON-RPC methods require data from the Canxium network, and fall neatly into three main categories: _Gossip, State, and History_. Use the links in these sections to jump to each method, or use the table of contents to explore the whole list of methods.
...
Returns the current Canxium protocol version. Note that this method is [not available in Geth](https://github.com/ethereum/go-ethereum/pull/22064#issuecomment-788682924).
...
`String` - The current Canxium protocol version
...
Returns `true` if client is actively mining new blocks. This can only return `true` for proof-of-work networks and may not be available in some clients since the fork.
...
Returns the number of hashes per second that the node is mining with. This can only return `true` for proof-of-work networks and may not be available in some clients since the fork.
...
3. `QUANTITY|TAG` - integer block number, or the string `"latest"`, `"earliest"`, `"pending"`, `"safe"`, `"finalized"`
...
1. `QUANTITY|TAG` - integer of a block number, or the string `"latest"`, `"earliest"`, `"pending"`, `"safe"` or `"finalized"`
...
The sign method calculates an Canxium specific signature with: `sign(keccak256("\x19Ethereum Signed Message:\n" + len(message) + message)))`.

By adding a prefix to the message makes the calculated signature recognizable as an Canxium specific signature. This prevents misuse where a malicious dapp can sign arbitrary data (e.g. transaction) and use the signature to impersonate the victim.
...
- `value`: `QUANTITY` - (optional) Integer of the value sent with this transaction
- `input`: `DATA` - (optional) Hash of the method signature and encoded parameters. For details see [Canxium Contract ABI in the Solidity documentation](https://docs.soliditylang.org/en/latest/abi-spec.html).
...
1. `QUANTITY|TAG` - a block number, or the string `"earliest"`, `"latest"`, `"pending"`, `"safe"` or `"finalized"`.
...
1. `QUANTITY|TAG` - a block number, or the string `"earliest"`, `"latest"`, `"pending"`, `"safe"`, `"finalized"`.
2. `QUANTITY` - the uncle's index position.
